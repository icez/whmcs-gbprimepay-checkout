...
class AS_Gbprimepay_API {
    ...
    public static function check_is_available()
    {
        ...
        if (
            ($payment_settings['enabled'] === 'no') && ($payment_settings_installment['enabled'] === 'no') &&
            ($payment_settings_qrcode['enabled'] === 'no') && ($payment_settings_qrcredit['enabled'] === 'no') && ($payment_settings_qrwechat['enabled'] === 'no') && ($payment_settings_barcode['enabled'] === 'no')
        ) {
            return 3;
        } else {
    ...
    public static function check_save_verified()
    {
        ...
        if (
            ($payment_settings['enabled'] === 'no') && ($payment_settings_installment['enabled'] === 'no') && 
            ($payment_settings_qrcode['enabled'] === 'no') && ($payment_settings_qrcredit['enabled'] === 'no') && 
            ($payment_settings_qrwechat['enabled'] === 'no') && ($payment_settings_barcode['enabled'] === 'no')
        ) {
            return 3;
        } else {
            if ($account_settings['environment'] === 'prelive') {
    ...
    public static function getSelectMethod($select_method)
    {
        if (!$select_method) {
            $select_method = "creditcard";
        }
    ...
    {
        $_currLang = $_SESSION['Language'] ? $_SESSION['Language'] : 'english';
        $_currentlanguage = 'English';
        if (($_currLang == "thai") || ($_currLang == "Thai")) {
    ...
    public static function getMerchantInfo($url, $configkey)
    {
        $account_settings = get_option('gbprimepay_account_settings');
    ...
    public static function sendMerchantCurl($url, $configkey, $field, $type)
    {
        if (empty($configkey)) {
            return false;
        }
    ...
    public static function afterpayCheckout($url, $configkey, $field, $type)
    {
        if (empty($configkey)) {
            return false;
        }
